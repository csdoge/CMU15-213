...
int isAsciiDigit(int x) {
  int checkHigh = 0x3 ^ (x >> 4);   //if high pos is 3, checkHigh is 0
  int newDigit = x + 6;   //check the low pos is less than 10(A)
  int checkLow = 0x3 ^ (newDigit >> 4);   //if low pos is lower than 10, checkLow is 0
...
int logicalNeg(int x) {
  x = (x >> 16) | x;  //make all the information suppressed to the last bit
  x = (x >> 8) | x;
  x = (x >> 4) | x;
  x = (x >> 2) | x;
  x = (x >> 1) | x;
